...
<article <?php blogmatic_schema_article_attributes(); ?> id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<?php
		if ( is_singular() ) :
			the_title( '<h1 class="entry-title" ' .blogmatic_schema_article_name_attributes(). '>', '</h1>' );
		else :
	...
	<?php blogmatic_post_thumbnail(); ?>

	<div <?php blogmatic_schema_article_body_attributes(); ?> class="entry-content">
		<?php
		the_content(
			sprintf(
				wp_kses(
