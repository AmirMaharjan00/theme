(function( api, $ ) {
    ...
    // set tab of control to design tab
    api.bind("ready", function() {
        api.control( 'video_playlist_border_radius' ).params.tab = 'design'
    })
    ...
    const builderHandler = {
        ...
        builderBehaviour: function( controlId ){
            let controlInstance = api.control( controlId )
            const { widgets } = controlInstance.params
        ...
        addActiveClasses: function(){
            ...
            api.section.each(( sectionInstance, sectionID ) => {
                sectionInstance.expanded.bind(function( isExpanded ){
                    if( isExpanded ) {
                        if( sectionInstance.contentContainer.hasClass( 'blogmatic-builder-related' ) ) {
                            let builderId = ''
                            ...
                            if( builderId !== '' ) {
                                ...
                                builderSettingsSection.parents( '#customize-controls' ).siblings('#customize-preview').addClass( 'blogmatic-builder--on' )
                            }
                        } else {
                            if( $('.blogmatic-builder.is-active') ) $('.blogmatic-builder.is-active').removeClass( 'is-active' )
                            if( $('.blogmatic-builder-related.active-builder-setting') ) $('.blogmatic-builder-related.active-builder-setting').removeClass( 'active-builder-setting' )
                            $('#customize-preview').removeClass( 'blogmatic-builder--on' )
                        }
                    }
                })
            })
