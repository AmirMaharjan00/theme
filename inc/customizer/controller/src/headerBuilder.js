...
import { BlogmaticControlHeader, convertNumbertoOrdinalString, blogmaticBackButtonClick, blogmaticReflectResponsiveInControl } from './component-function'
...
export const BlogmaticHeaderBuilder = ( props ) => {
    const [ value, setValue ] = useState( props.value )
    const [ activeContainer, setActiveContainer ] = useState( null )
    const { label, description, widgets, placement, builder_settings_section: builderSettingsSection } = customize.settings.controls[props.setting]
    const [ isBuilderHidden, setIsBuilderHidden ] = useState(  )
    const [ activeWidget, setActiveWidget ] = useState( null )
    const [ activeIndicator, setActiveIndicator ] = useState( null )
    const [ responsive, setResponsive ] = useState( 'desktop' )
    const { setHeaderBuilderReflector, setFooterBuilderReflector } = useDispatch( myCustomStore );
    ...
    useEffect(() => {
        ...
        const rowSections = rowCount.map( row => placement + '_' + convertNumbertoOrdinalString( row ) + '_row' )
        blogmaticBackButtonClick( builderSettingsSection, [ ...rowSections, ...widgetsSection ], setActiveWidget, setActiveIndicator )
    }, [])
                                        ...
                                        <div className='column-wrapper'>
                                            {
                                                Object.entries( row ).map(( [ containerId, columnWidgets ], columnIndex ) => {
                                                    ...
                                                    return <Dropdown
                                                        ...
                                                        contentClassName = "blogmatic-header-builder-popover"
...
export const RowSettings = ( props ) => {
    ...
    return <div className={ _thisClass } onClick={ handleOnClick }>
        ...
        { ( label !== '' ) && <span>{ __( escapeHTML( label ), 'blogmatic-pro' ) }</span> }
    </div>
}
...
export const Footer = ( props ) =>  {
    const { isBuilderHidden } = props

    /* Toggle the builder */
    const toggleBuilder = () => {
        const mainContainer = document.getElementById('customize-controls')
        if( mainContainer.classList.contains( 'blogmatic-builder-collapsed' ) ) {
            mainContainer.classList.remove( 'blogmatic-builder-collapsed' )
            props.setIsBuilderHidden( false )
        } else {
            mainContainer.classList.add( 'blogmatic-builder-collapsed' )
