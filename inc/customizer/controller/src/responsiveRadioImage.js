...
import { BlogmaticControlHeader, blogmaticReflectResponsiveInControl, blogmaticReflectResponsiveInCustomizer, BlogmaticGetResponsiveIcons, convertNumbertoCardinalString } from './component-function'

export const BlogmaticResponsiveRadioImage = ( props ) => {
   ...
   const columnCounts = useSelect(( select ) => {
    ...
    useEffect(() => {
        ...
        if( ! layoutIds.includes( desktop ) ) setValue({ ...value, desktop: 'one' })
        if( ! layoutIds.includes( tablet ) ) setValue({ ...value, tablet: 'one' })
        if( ! layoutIds.includes( smartphone ) ) setValue({ ...value, smartphone: 'one' })
    }, [ columnCounts ])


    return <div className='radio-image-wrapper'>
        <BlogmaticControlHeader label={ label } description={ description }>
            <BlogmaticGetResponsiveIcons responsive={ responsive } stateToSet={ handleResponsiveIconsClick } />
