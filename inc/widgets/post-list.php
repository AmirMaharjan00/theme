 ...
 */
 ...
 class Blogmatic_Post_List_Widget extends WP_Widget {
    public function __construct() {
        parent::__construct(
            'blogmatic_post_list_widget',
            esc_html__( 'Blogmatic: Post List', 'blogmatic-pro' ),
            [ 'description' => __( 'A collection of post in a List', 'blogmatic-pro' ) ]
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $layout = ( isset( $instance['layout'] ) ) ? $instance['layout'] : 'one';
        ...
        echo wp_kses_post( $before_widget );
            ...
            $post_list_query = new \WP_Query( apply_filters( 'blogmatic_query_args_filter', $post_list_args ) );
            if( $post_list_query->have_posts() ) :
                ...
                if( $ajax_load_more_option && $post_list_query->max_num_pages > 1 ) :
                    ...
                    <div class="blogmatic-widget-loader">
                            <button class="load-more" data-widget="blogmatic_post_list_widget" data-instance="<?php echo esc_attr( json_encode( $instance ) ); ?>"><?php echo esc_html__( 'Show more', 'blogmatic-pro' ); ?></button>
                        </div>
                    <?php
                endif;
            endif;
    ...
    public function widget_fields() {
        ...
        return [
            [
                'name'  =>  'widget_title',
                'type'  =>  'text',
                'title' =>  esc_html( 'Widget Title', 'blogmatic-pro' ),
                'description'   =>  esc_html__( 'Add the widget title here', 'blogmatic-pro' ),
                'default'   =>  esc_html__( 'Post List', 'blogmatic-pro' )
            ],
            [
                'name'  =>  'post_catgories',
                'type'  =>  'select-two',
                'title' =>  esc_html__( 'Post Categories', 'blogmatic-pro' ),
            ...
            [
                'name'  =>  'post_tags',
                'type'  =>  'select-two',
                'title' =>  esc_html__( 'Post Tags', 'blogmatic-pro' ),
                'description'   =>  esc_html__( 'Choose the tag to display list of posts', 'blogmatic-pro' ),
                'options'   =>  $tag_list
            ],
            [
                'name'  =>  'post_authors',
                'type'  =>  'select-two',
                'title' =>  esc_html__( 'Post Authors', 'blogmatic-pro' ),
                'description'   =>  esc_html__( 'Choose the author to display list author posts', 'blogmatic-pro' ),
                'options'   =>  $user_list
            ],
            [
                'name'  =>  'post_to_include',
                'type'  =>  'select-two',
                'title' =>  esc_html__( 'Post to Include', 'blogmatic-pro' ),
                'description'   =>  esc_html__( 'Choose the posts to display in the list of posts', 'blogmatic-pro' ),
                'options'   =>  $post_list
            ],
            [
                'name'  =>  'post_to_exclude',
                'type'  =>  'select-two',
                'title' =>  esc_html__( 'Post to Exclude', 'blogmatic-pro' ),
                'description'   =>  esc_html__( 'Choose the posts to exclude from list of posts', 'blogmatic-pro' ),
                'options'   =>  $post_list
            ],
            [
            ...
            [
                'name'  =>  'layout',
                'type'  =>  'select',
                'title' =>  esc_html__( 'layout', 'blogmatic-pro' ),
                'options'   =>  [
            ...
            [
                'name'  =>  'image_size',
                'type'  =>  'select',
                'title' =>  esc_html__( 'Image Size', 'blogmatic-pro' ),
                'options'   =>  blogmatic_get_image_sizes_option_array(),
                'default'   =>  'medium'
            ]
        ];
    }
